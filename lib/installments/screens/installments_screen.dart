...
class _PaymentHistoryDialog extends StatelessWidget {
        ...
        child: SizedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              _summaryRow('Invoice ID',      '#${plan.invoiceId}'),
              const SizedBox(height: 20),
              ...
              if (!plan.isCompleted && plan.status != 'cancelled')
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () => _confirmCancel(context, plan),
                    icon: const Icon(Icons.cancel_presentation, size: 18),
                    label: const Text('Cancel Plan (Item Returned)'),
                    style: ElevatedButton.styleFrom(
  ...
  void _confirmCancel(BuildContext context, DbInstallmentPlan plan) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF1E293B),
        title: const Text('Cancel Installment Plan', style: TextStyle(color: Colors.white)),
        content: const Text(
          'Are you sure you want to cancel this plan?\nThis is typically used when an item is returned. The remaining debt will be removed from the client.',
          style: TextStyle(color: Color(0xFF94A3B8)),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(ctx).pop(),
            child: const Text('No, Keep Plan', style: TextStyle(color: Color(0xFF94A3B8))),
          ),
          TextButton(
            onPressed: () async {
              Navigator.of(ctx).pop();
              final success = await AppDataStore.instance.cancelInstallmentPlan(installmentId: plan.id);
              if (success && context.mounted) {
                Navigator.of(context, rootNavigator: true).pop(); // Close main dialog
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                  content: Text('Plan successfully cancelled.'),
