    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text(
            'Plan created for ${_selectedClient!.name} — ${_selectedProduct!.name}.',
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              // ── Item Name ────────────────────────────────────────────────
              _label('Select Product', required: true),
              const SizedBox(height: 8),
              ValueListenableBuilder<List<DbProduct>>(
                valueListenable: AppDataStore.instance.productsNotifier,
                builder: (context, products, _) {
                  return _dropdown<DbProduct>(
                    ...
                    onChanged: (p) => setState(() {
                      _selectedProduct = p;
                      if (p != null) {
                        _totalCtrl.text = p.sellingPrice.toStringAsFixed(2);
                      }
                      _error = null;
                    }),
                  );
